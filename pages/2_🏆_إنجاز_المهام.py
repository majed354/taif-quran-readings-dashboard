...
def get_member_list(achievements_df):
    ...
    if not achievements_df.empty and "اسم العضو" in achievements_df.columns:
        ...
        # إرجاع قائمة فارغة في حالة عدم وجود بيانات
        st.info("لا توجد بيانات لأعضاء هيئة التدريس")
        return []
...
def get_available_years(achievements_df):
    ...
    if not achievements_df.empty and "التاريخ" in achievements_df.columns:
        ...
        # إرجاع العام الحالي فقط في حالة عدم وجود بيانات
        current_year = datetime.now().year
        st.info(f"لا توجد بيانات سنوية متاحة. سيتم استخدام السنة الحالية {current_year}")
        return [current_year]
...
def get_main_tasks_list(achievements_df):
    ...
    if not achievements_df.empty and "المهمة الرئيسية" in achievements_df.columns:
        ...
        # إرجاع قائمة بها الخيار الافتراضي فقط في حالة عدم وجود بيانات
        st.info("لا توجد بيانات للمهام الرئيسية")
        return ["— بدون مهمة رئيسية —"]
# =========================================
