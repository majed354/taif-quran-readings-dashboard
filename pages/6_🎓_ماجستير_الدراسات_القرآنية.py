...
# --- إعدادات الصفحة ---
st.set_page_config(
    page_title="ماجستير الدراسات القرآنية المعاصرة | قسم القراءات",
    page_icon="🎓",
    layout="wide"
)
...
st.markdown("<h1>🎓 ماجستير الدراسات القرآنية المعاصرة</h1>", unsafe_allow_html=True)
...
# =========================================
    ...
    # صفحات البرامج الأكاديمية
    program_tabs = st.tabs(["الملخّص التنفيذي", "مؤشّرات البرنامج", "الخطة الدراسيّة والمقرّرات", "التقارير والوثائق"])
    with program_tabs[0]:
        st.info("محتوى الملخص التنفيذي سيضاف لاحقاً")
    with program_tabs[1]:
        st.info("محتوى مؤشرات البرنامج سيضاف لاحقاً")
    with program_tabs[2]:
        st.info("محتوى الخطة الدراسية سيضاف لاحقاً")
    with program_tabs[3]:
        st.info("محتوى التقارير سيضاف لاحقاً")


# مثال لمحتوى بديل إذا لم تكن هناك تبويبات
# if not ("ماجستير_الدراسات_القرآنية" == "هيئة_التدريس" or "ماجستير_الدراسات_القرآنية" == "إنجاز_المهام" or "master_contemporary" is not None):
#     st.write(f"محتوى صفحة 'ماجستير الدراسات القرآنية المعاصرة' قيد الإنشاء.")
